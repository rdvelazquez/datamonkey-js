...
// To be renamed to "removeTreeFromNexusOrFasta"... currently getting "issue removing tree from file" error on fel... see line 95 of app/routes/fel.js.
  ...
  return new Promise(function(resolve, reject) {
    fs.readFile(file_path, function(err, data) {
      ...
      if (file_lines[0] != "#NEXUS") {
        ...
        fs.writeFile(output_file_path, FastaStringWithoutTree, function(err){
          if(err) reject(err);
          resolve();
        });
      } else {
        ...
        fs.writeFile(output_file_path, NexusStringWithoutTree, function(err){
          if(err) reject(err);
          resolve();
        });
      }
...
Msa.statics.removeTreeFromNexus_Old = function(input_file_path, output_file_path){
  return new Promise(function(resolve, reject) {
